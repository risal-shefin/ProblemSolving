...
struct Hash {
    ...
    inline void add(ll ch, ll i) {
        if(i==1) h1[0] = h2[0] = 0;

        h1[i] = (h1[i-1] * base1 + ch) % mod;
        h2[i] = (h2[i-1] * base2 + ch) % mod;
    }
...
int main()
{
    ...
    ll idx = 0, cnt = 0, ans = 0, ok = 1;
    for1(i, m) {
        ll y; sl(y);

        for(ll i = idx+1; i < y; i++) {
            fw2.add('?', i);
            cnt++;
        }
        ...
        if(idx >= y) {
            ll len = idx-y+1;
            if(fw.hashVal(1, len) != fw2.hashVal(y, idx)) {
                ans = ok = 0;
                break;
            }
        }

        ll start = max(1LL, idx-y+1 +1);
        ll cur = y+start-1;

        for(ll i = start; i <= slen; i++) {
            fw2.add(s[i], cur);
            cur++;
        }
        idx = y+slen-1;
